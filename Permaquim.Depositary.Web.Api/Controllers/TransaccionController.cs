                ...
                if (model.SynchronizationExecutionId.HasValue)
                {
                    ...
                    if (EjecucionId.HasValue)
                    {
                        if (model.Contenedores.Count > 0)
                        {
                            ...
                            Int64? SincronizacionContenedorId = SynchronizationController.iniciarCabeceraSincronizacion(EjecucionId.Value, ENTIDAD_CONTENEDOR);
                        ...
                        {
                            //Iniciamos un registro de sincronizacion de la entidad
                            Int64? SincronizacionSesionId = SynchronizationController.iniciarCabeceraSincronizacion(EjecucionId.Value, ENTIDAD_SESION);
                        ...
                        {
                            //Iniciamos un registro de sincronizacion de la entidad
                            Int64? SincronizacionCierreDiarioId = SynchronizationController.iniciarCabeceraSincronizacion(EjecucionId.Value, ENTIDAD_CIERREDIARIO);
                        ...
                        {
                            //Iniciamos un registro de sincronizacion de la entidad
                            Int64? SincronizacionTurnoId = SynchronizationController.iniciarCabeceraSincronizacion(EjecucionId.Value, ENTIDAD_TURNO);
                        ...
                        {
                            //Iniciamos un registro de sincronizacion de la entidad
                            Int64? SincronizacionTransaccionId = SynchronizationController.iniciarCabeceraSincronizacion(EjecucionId.Value, ENTIDAD_TRANSACCION);
                            ...
                            try
                            {
                                foreach (var transaccionItem in model.Transacciones)
                                {
                                    ...
                                    DepositaryWebApi.Entities.Tables.Operacion.Transaccion? newTransaccionEntity;

                                    //Si existia la transaccion le mapeamos el id 
                                    if (idTransaccionExistente.HasValue)
                                        transaccionItem.Id = idTransaccionExistente.Value;

                                    newTransaccionEntity = transaccionController.AddOrUpdate(transaccionItem);
                                    ...
                                    if (newTransaccionEntity != null)
                                    {
                                        ...
                                        if (existenTransaccionesDetalles)
                                        {
                                            ...
                                            if (!SincronizacionTransaccionDetalleId.HasValue)
                                                SincronizacionTransaccionDetalleId = SynchronizationController.iniciarCabeceraSincronizacion(EjecucionId.Value, ENTIDAD_TRANSACCIONDETALLE);
                                        ...
                                        else if (existeTransaccionSobre)
                                        {
                                            ...
                                            if (!SincronizacionTransaccionSobreId.HasValue)
                                                SincronizacionTransaccionSobreId = SynchronizationController.iniciarCabeceraSincronizacion(EjecucionId.Value, ENTIDAD_TRANSACCIONSOBRE);
                                                ...
                                                if (newTransaccionSobreEntity != null)
                                                {
                                                    ...
                                                    if (existenTransaccionesSobresDetalles)
                                                    {
                                                        ...
                                                        if (!SincronizacionTransaccionSobreDetalleId.HasValue)
                                                            SincronizacionTransaccionSobreDetalleId = SynchronizationController.iniciarCabeceraSincronizacion(EjecucionId.Value, ENTIDAD_TRANSACCIONSOBREDETALLE);
